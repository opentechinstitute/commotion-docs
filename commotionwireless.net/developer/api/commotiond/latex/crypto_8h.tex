...
serval-\/dna plugin functionality for signing/verifying using Serval keys \begin{DoxyAuthor}{Author}
...
\begin{DoxyCode}
574 \{
575   \textcolor{keywordtype}{int} ret = 0;
576   \hyperlink{structsvl__crypto__ctx}{svl\_crypto\_ctx} *ctx = svl\_crypto\_ctx\_new();
577   \textcolor{comment}{}
578 \textcolor{comment}{  /** skipping some error checking for performance reasons */}
579   
580 \textcolor{comment}{//   CHECK(IS\_LIST(params) && co\_list\_length(params) == 2, "Invalid params");}
581   
582   ctx->msg\_len = co\_obj\_data((\textcolor{keywordtype}{char}**)&ctx->msg, \hyperlink{list_8c_a647ff4c713547b65d3fa7a1a7cc7dff7}{co\_list\_element}(params, 1));
583   
584   memcpy(ctx->sas\_private,\_LIST\_ELEMENT(params, 0),crypto\_sign\_SECRETKEYBYTES);
585   
586   CHECK(serval\_create\_signature(ctx), \textcolor{stringliteral}{"Failed to sign OLSRd packet"});
587   
588   CMD\_OUTPUT(\textcolor{stringliteral}{"sig"}, co\_bin8\_create((\textcolor{keywordtype}{char}*)ctx->signature, SIGNATURE\_BYTES, 0));
589   
590   ret = 1;
591 error:
592   svl\_crypto\_ctx\_free(ctx);
